...
class Hit_ray:
    ...
    def check(self, hit_callback, distance, current_block, next_block):
        if self.world.get_block_number(next_block):
            ...
            self.position = list(
                map(lambda x: self.position[x] + self.vector[x] * distance, range(3))
            )
    ...
    def step(self, hit_callback):
        ...
        if vx:
            ...
            if y >= -0.5 and y <= 0.5 and z >= -0.5 and z <= 0.5:
                ...
                return self.check(
                    hit_callback, distance, (bx, by, bz), (bx + sign[0], by, bz)
                )
        ...
        if vy:
            ...
            if x >= -0.5 and x <= 0.5 and z >= -0.5 and z <= 0.5:
                distance = math.sqrt((x - lx) ** 2 + (y - ly) ** 2 + (z - lz) ** 2)
                return self.check(
                    hit_callback, distance, (bx, by, bz), (bx, by + sign[1], bz)
                )
        ...
        if vz:
            ...
            if x >= -0.5 and x <= 0.5 and y >= -0.5 and y <= 0.5:
                distance = math.sqrt((x - lx) ** 2 + (y - ly) ** 2 + (z - lz) ** 2)
                return self.check(
                    hit_callback, distance, (bx, by, bz), (bx, by, bz + sign[2])
                )
